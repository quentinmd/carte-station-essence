...
export const stationSlug = (record) => {
  const name = record.enseigne || record.nom_station || record.nom || record.adresse || 'station'
...
export const stationSeoUrl = (record) => `${SITE_URL}/station/${stationSlug(record)}`
...
export const extractStationSummary = (record) => {
  ...
  return {
    ...
    name: record.enseigne || record.nom_station || record.nom || `Station ${city || record.cp || ''}`.trim(),
    city,
    address,
